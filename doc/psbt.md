# PSBT Howto for onyx Core

Since onyx Core 2.0.0, an RPC interface exists for Partially Signed onyx
...
PSBT is an interchange format for onyx transactions that are not fully signed
...
Overall, the construction of a fully signed onyx transaction goes through the
following steps:
...
- An **Extractor** produces a valid onyx transaction (in network format)
...
#### Multisig with multiple onyx Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
onyx Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `onyx-cli` is used.
...
Later, when *V* onyx has been received on *Amulti*, and Bob and Carol want to
