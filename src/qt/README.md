This directory contains the onyxQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the onyx UI.
...
ClientModel is used by the main application `onyxgui` and several models like `peertablemodel`.
...
Handles URI based application switching (e.g. when following a onyx:... link from a browser).
...
* `bitcoinunits.(h/cpp)`: onyx / monyx / etc handling
* `callback.h`
...
4. Enter "onyx-qt" as project name, enter src/qt as location
