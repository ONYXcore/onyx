 ...
 * between onyxd, and onyx-qt and non-server tools.
*/
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ONYXcore/onyx>";
    const std::string URL_WEBSITE = "<https://onyx.run>";
    ...
    return CopyrightHolders(strprintf(_("Copyright (C) %i-%i"), 2017, COPYRIGHT_YEAR) + " ") + "\n" +
           ...
           "\n" +
           "\n" +
           _("Block Explorer: <https://explorer.onyx.run>") + "\n" +
           strprintf(_("Mining Pools: %s | %s | %s"), "<https://pool-eu.onyx.run>", "<https://pool-us.onyx.run>", "<https://pool-as.onyx.run>") + "\n" +
           "\n" +
           strprintf(_("DISCORD | TELEGRAM: %s - %s"), "<https://onyx.run/telegram>", "<https://onyx.run/discord>") +
           "\n";
}
...
/** Sanity checks
 *  Ensure that onyx is running in a usable environment with all
...
{
    // Make sure only a single onyx process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if onyx is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if onyx is started while in a temporary directory.\n",
