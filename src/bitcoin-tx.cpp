...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " onyx-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  onyx-tx [options] <hex-tx> [commands]  Update hex-encoded onyx transaction\n" +
            "or:     onyx-tx [options] -create [commands]   Create hex-encoded onyx transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded onyx transaction
